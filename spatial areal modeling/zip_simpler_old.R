...
##### setting up z
# zero_location <- c(which(count_var > 0))
z_init <- rep(1, length(count_var))
z_init[-count0_locator] <- 0

###### setting up initial beta2
beta2_init <- unname(lm(log(count_var[z_init == 0] - 1 +.25) ~ covariate_data[z_init == 0, ])$coef) # BETA2 initial value

###### setting up initial log_lambda
log_lambda_init <- numeric(length(count_var))
log_lambda_init[z_init == 0] <- log(count_var[z_init == 0] -1 +.25)
...
zee = z_init; omega = omega_init
zee0_locator <- which(zee==0); zee1_locator <- which(zee==1)
...
A_sigma2 = (length(count_var[zee0_locator])/2) + a_sigma_pr                 # posterior scale parameter for sigma2
...
ncol_tempM = length(c(1, 1, beta1, beta2, sigma2, tau2_theta1, tau2_theta2, theta1, theta2, omega, log_lambda, log_lambda, log_lambda))
...
for (j in 1:num_iter) {
  ...
  ###### Update theta2: 
  res_nonspatial_poisson <-  log_lambda[zee0_locator] - X[zee0_locator, ] %*% beta2 # (log_lambda- Xb) part in theta2 posterior
  ...
  for(i in 1: length(zee0_locator)) {  # update depends on prior and likelihood
    theta2_position <- zee0_locator[i]
  ...
  for(i in 1: length(zee1_locator)) { # update depends on prior only
    theta2_position <- zee1_locator[i]
  ...
  ss_poisson <- sum((res_nonspatial_poisson - theta2[zee0_locator])^2) # res_nonspatial = log_lambda - X_train*beta
  ...
  x_transpose_sigma_inv_beta2 = (sigma2^-1) * tX[, zee0_locator]
  A_beta2 = x_transpose_sigma_inv_beta2 %*% X[zee0_locator, ] + sigma_beta2_inv; A_beta2_inv = chol2inv(chol(A_beta2))
  B_beta2 = x_transpose_sigma_inv_beta2 %*% (log_lambda[zee0_locator] - theta2[zee0_locator]) + sigma_beta2_inv %*% mu_beta2
  ...
  count_prob <- numeric(length(count_var))
  
  # for non-zero location: poisson
  count_prob[-count0_locator] <-  dpois(count_var[zee0_locator]-1, lambda = current_lambda[zee0_locator]) * (1- phi[zee0_locator])
  # for those location has count 0: Bernoulli
  count_prob[count0_locator] <- phi[zee1_locator]
  lg_lkhd <- sum(log(count_prob))
  
  o_non0 <- numeric(length(count_var))
  o_non0[-count0_locator] <- log(dpois(count_var[zee0_locator]-1, lambda = current_lambda[zee0_locator]))
  o_non0[count0_locator] <- NA
  
  phi_part <- numeric(length(count_var))
  phi_part[-count0_locator] <- log((1- phi[zee0_locator]))
  phi_part[count0_locator] <- log(phi[zee1_locator])
  ...
  tempM[j, ] <-  c(lg_lkhd, acc_rate_per_iteration, beta1, beta2, sigma2, tau2_theta1, tau2_theta2, theta1, theta2, omega, log_lambda, o_non0, phi_part)
...
output <- as.data.frame(tempM) %>%
  ...
  set_names(c("lg_lkhd", "acc_rate_iter",  paste0("beta1_", 0:p), paste0("beta2_", 0:p),  "sigma2", "tau2_theta1", "tau2_theta2", paste0("theta1_", 1:n), paste0("theta2_", 1:n), paste0("omega_", 1:n),
              paste0("log_lambda", 1:n), paste0("o_non0", 1:n),  paste0("phi_", 1:n)))
...
for( i in 1:2) { 
  ...
  temp <- data.frame(model_name = "ZIP_simpler", 
...
count_prob_bar <- numeric(length(count_var))
count_prob_bar[-count0_locator] <-  dpois(count_var[zee0_locator]-1, lambda = exp(parameter_summary[parameter_summary$parameter %in% paste0("log_lambda", zee0_locator), "mean"])) * (1- phi_bar[zee0_locator])
count_prob_bar[count0_locator] <- phi_bar[zee1_locator]
...
performance_metric <- data.frame(model_name = "ZIP_simpler", 
...
# file_name <- paste0(substr(count_var_name, start = 3, stop = nchar(count_var_name)), "_zipSimpler_sd_", mh_sd, "acc_", acc)
file_name <- paste0(substr(count_var_name, start = 3, stop = nchar(count_var_name)), "_zipSimpler")
#=========================================================================
