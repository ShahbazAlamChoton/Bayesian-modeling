...
for(fold in 1:cv_count) {
  ...
  count_col <- rawData$count_data[, current_response] # All data (train + test) for a give count column
  count_var <- count_col[train_loc] # contains training count data
  count_test <- count_col[test_loc] # contains test count data
  ...
  beta2_init <- unname(lm(log(count_var[z_init == 0] + 0.25) ~ covariate_data[train_loc, ][z_init == 0, ])$coef) # BETA2 initial value
  # is.na(beta2_init) <- 0
  ...
  #output matrix
  # length(c(beta1, theta1, beta2, theta2)) # 84 -> p = 5, data_row = 36; 6 + 36 + 6 + 36 = 84
  #  ncol_tempM = c(test_loc, lambda_test, mu_test, y_tilda_test, omega_test, z_test, predicted_count, beta1, theta1, beta2, theta2, acc_rate_per_iteration)
  ncol_tempM = 127
  ...
  for (j in 1:num_iter) {
    ...
    proposed_log_lambda <- rnorm(length(zee0_locator), mean = log_lambda[zee0_locator], sd = mh_sd) # sd can be changed
    ...
    ###### Update theta2: 
    res_nonspatial_poisson <-  log_lambda - X_train %*% beta2 # (log_lambda- Xb) part in theta2 posterior
    ...
    ss_poisson <- sum((res_nonspatial_poisson - theta2[train_loc])^2) # res_nonspatial = log_lambda - X_train*beta
    ...
    x_transpose_sigma_inv_beta2 = (sigma2^-1) * tX_train
    A_beta2 = x_transpose_sigma_inv_beta2 %*% X_train + sigma_beta2_inv; A_beta2_inv = chol2inv(chol(A_beta2))
    B_beta2 = x_transpose_sigma_inv_beta2 %*% (log_lambda - theta2[train_loc]) + sigma_beta2_inv %*% mu_beta2
    beta2 <- mvrnorm(1, A_beta2_inv %*% B_beta2, A_beta2_inv)
  
    ###################################### 2nd part: bernoulli
    ###### Update zee:
    zee[-count0_locator] = 0 # for non-zero count, data surely comes from Poisson
    phi_count0_location <- phi[count0_locator] # required, to know which zero counts comes from poisson part
    ...
    zee[count0_locator] <- if_else(runif(length(count0_locator)) <= post_prob_zee1, 1, 0) # reveals zero counts distribution (which from poisson, which from Bernoulli(mu))
    ...
    ###### Update theta1: 
    # mu_omega <- X_train %*% beta1
    res_nonspatial_omega <-  omega - mu_omega # (omega- X * beta1) part of calculation in theta1 posterior
    ...
    #============================== Prediction
    # Poisson part:
    lambda_test <- exp(rnorm(n_test, mean = X_test %*% beta2 + theta2[test_loc], sd = sqrt(sigma2)))
    y_tilda_test <- rpois(n_test, lambda = lambda_test)
    
    # 2nd part:
    mu_test <- X_test %*% beta1 + theta1[test_loc] # mean of 2nd part of the model (Bernoulli part): X(test) * Beta1 + Theta_1(test)
    omega_test <- rnorm(n_test, mean = mu_test, sd = 1) # Generating test W [W ~ Trunc Normal(X*Beta1 + theta_1, 1)]
    z_test <- if_else(omega_test > 0, 1, 0) # Generating test Z; if W > 0 Z = 1 otherwise Z = 0
    # Prediction for test data using ZIP: count = (1 - Z_i) * Y_tilda_i;
    # if Z = 0 -> count comes from Poisson part; if Z = 1 -> count = 0
    predicted_count <- if_else(z_test == 0, y_tilda_test, 0) 
    #==================================== Prediction End ======================
    
    # Accumulates all test estimates for a single iteration
    # ncol_tempM = length(c(lambda_test, y_tilda_test, omega_test, z_test))
    tempM[j, ] <-  c(test_loc, lambda_test, mu_test, y_tilda_test, omega_test, z_test, predicted_count, beta1, theta1, beta2, theta2, acc_rate_per_iteration)
    ...
    rm(acc_rate_per_iteration, lambda_acc, proposed_log_lambda, 
       ...
       lambda_test, y_tilda_test, omega_test, z_test, predicted_count) 
    } 
  
  # c(test_loc, lambda_test, mu_test, y_tilda_test, omega_test, z_test, predicted_count, beta1, theta1, beta2, theta2, acc_rate_per_iteration)
  output <- as.data.frame(tempM) %>%
    ...
    set_names(c(paste0("test_loc_", 1:n_test), paste0("lambda_test_", 1:n_test), paste0("mu_test_", 1:n_test), paste0("ytilda_test_", 1:n_test), 
                ...
                paste0("beta1_", 0:p), paste0("theta1_", 1:n), paste0("beta2_", 0:p), paste0("theta2_", 1:n), "mean_acc_rate" )) %>% 
  ...
  prediction_temp <- data.frame(fold_number = fold, 
                                ...
                                median_prediction = apply(output[, paste0("predicted_count_", 1:n_test)], 2, median),
                                zee1_prop = apply(output[, paste0("zee_test_", 1:n_test)], 2, mean),
                                y_tilda_mean = apply(output[, paste0("ytilda_test_", 1:n_test)], 2, mean),
                                y_tilda_median = apply(output[, paste0("ytilda_test_", 1:n_test)], 2, median),
                                mean_prediction = apply(output[, paste0("predicted_count_", 1:n_test)], 2, mean),
                                pred_q05 = apply(output[, paste0("predicted_count_", 1:n_test)], 2, quantile, .05),
                                pred_q95 = apply(output[, paste0("predicted_count_", 1:n_test)], 2, quantile, .95)
                                   ) %>% 
...
prob_being_count0 <- pnorm(mu_test_summary) + (1 - pnorm(mu_test_summary))* elambda_test_matrix
...
file_name <- paste0(substr(count_var_name, start = 3, stop = nchar(count_var_name)), "_zip_cv")
