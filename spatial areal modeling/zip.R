...
num_iter = 50000; burn_in = 20000; thin_in = 10
...
beta2_init <- unname(lm(log(count_var[z_init == 0] + 0.25) ~ covariate_data[z_init == 0, ])$coef) # BETA2 initial value
...
ncol_tempM = length(c(1, 1, 1, beta1, beta2, sigma2, tau2_theta1, tau2_theta2, theta1, theta2, omega, log_lambda, which(count_var == 0))) 
...
for (j in 1:num_iter) {
  ...
  count_prob_new <- numeric(length(count_var))
  count_prob_new[zee0_locator] <-  dpois(count_var[zee0_locator], lambda = current_lambda[zee0_locator]) * (1- phi[zee0_locator])
  # for those location has count 0: Bernoulli
  count_prob_new[zee1_locator] <- phi[zee1_locator]
  lg_lkhd_new <- sum(log(count_prob_new))
  ...
  tempM[j, ] <-  c(lg_lkhd,lg_lkhd_new, acc_rate_per_iteration, beta1, beta2, sigma2, tau2_theta1, tau2_theta2, theta1, theta2, omega, log_lambda, zee[count_var == 0])
...
output <- as.data.frame(tempM) %>%
  ...
  set_names(c("lg_lkhd", "lg_lkhd_new", "acc_rate_iter",  paste0("beta1_", 0:p), paste0("beta2_", 0:p),  "sigma2", "tau2_theta1", "tau2_theta2", paste0("theta1_", 1:n), paste0("theta2_", 1:n), paste0("omega_", 1:n),
...
for( i in 1:2) { 
  ...
  temp <- data.frame(model_name = "ZIP", 
...
performance_metric <- data.frame(model_name = "ZIP", 
...
file_name <- paste0(substr(count_var_name, start = 3, stop = nchar(count_var_name)), "_zip")
#==========================================================================
...
# xxxxxxxxxxxxxxxxxxxx END xxxxxxxxxxxxxxxxxxxx
